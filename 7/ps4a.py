...
def getWordScore(word, n):
    ...
    returns: int >= 0
    """
    score_sum = sum(SCRABBLE_LETTER_VALUES[letter] for letter in word)
    score_final = score_sum * len(word)
    if len(word) == n:
        score_final += 50
...
def updateHand(hand, word):
    ...
    for letter in hand:
        if letter in word_dict:
            hand_dict[letter] -= word_dict[letter]
    return hand_dict
...
def isValidWord(word, hand, wordList):
    ...
    for letter in word_dict:
        if word_dict[letter] > hand_dict.get(letter, 0):
            return False
    return True
...
def calculateHandlen(hand):
    ...
    returns: integer
    """
    length = sum(hand[key] for key in hand)
    return length
...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    total_score = 0
    initial_hand_length = calculateHandlen(hand)
    ...
    while calculateHandlen(hand):
        ...
        # Otherwise (the input is not a single period):
            ...
            # Otherwise (the word is valid):
                ...
                word_score = getWordScore(word, initial_hand_length)
