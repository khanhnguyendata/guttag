...
def r_squared(y, estimated):
    ...
    """
    y_mean = sum(y) / len(y)
    nominator = sum((y_i - e_i)**2 for y_i, e_i in zip(y, estimated))
    denominator = sum((y_i - y_mean)**2 for y_i in y)
...
def evaluate_models_on_training(x, y, models):
    ...
    for model in models:
        estimated_ys = []
        for x_i in x:
            estimated_y = sum(coefficient * (x_i**degree) for degree, coefficient in enumerate(model[::-1]))
            estimated_ys.append(estimated_y)
        r_squares[len(model)] = r_squared(y, estimated_ys)
        pylab.plot(x, estimated_ys, 'r-', label='Estimated (degree {})'.format(len(model)))

    pylab.plot(x, y, 'b.', label='Actual')
    title1 = 'Temperature of Jan 10th from {} to {}\n'.format(x[1], x[-1])
    for model in models:
        title1 += 'R^2 (degree {}): {}\n'.format(len(model), r_squares[len(model)])
    pylab.title(title1[:-1])
    pylab.xlabel('Year')
...
models = generate_models(x1, y, [50])
